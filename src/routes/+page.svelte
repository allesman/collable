...
// @ts-nocheck
  ...
  // for gameStage 0
  // let artistId= 683879; // id of the current artist, deprecated
  // let artistName = "Quadeca"; // name of the current artist, deprecated
  ...
  let songName = null; // the name of the song selected by the user
  ...
  // async function handleClickSongForm(event) {
  //   event.preventDefault();
  //   const formData = new FormData(event.target);
  //   const response = await fetch(event.currentTarget.action, {
  //     method: 'POST',
  ...
  async function handleClickArtist(event){
    gameStage = 0;
    const index = event.currentTarget.dataset.index;
    // artistId = event.currentTarget.dataset.artistid;
    // artistName = event.currentTarget.dataset.artistname;
...
<div class="flex items-center justify-center">
  <form method="POST" on:submit|preventDefault={handleSearch} action="?/search" autocomplete="off" class="w-full max-w-xs">
    <!-- TODO: prevent from typing after submission -->
    <input name="songQuery" type="text" placeholder="Enter Song :3" disabled={isLoading||gameStage!=0} class="input input-bordered select-none" />
    <input type="hidden" name="artistId" bind:value={artistObj.id} />
    <!-- <div class="flex items-center justify-center"> -->
      <button type="submit" disabled={isLoading||gameStage!=0} class="btn mt-2 btn-primary">Search</button>
    <!-- </div> -->
  </form>
</div>
        ...
        <ul class="flex flex-col items-center justify-center">
          {#each searchResults as hit,i}
            <li class="w-full text-center m-1">
              <!-- <form action="?/clickSong" method="POST" on:submit|preventDefault={handleClickSong} > -->
                <!-- <input type="hidden" name="songId" value={hit.result.id} /> -->
                <button on:click={handleClickSong} data-index={i} class="btn btn-secondary btn-outline">
                  <!-- add image -->
                  {hit.result.title}
                  <!-- TODO: iterate through primary and features artists -->
                  <span class="badge badge-secondary">{hit.result.artist_names}</span> 
                </button>
              <!-- </form> -->
            </li>
          {/each}
  ...
  <div class="flex flex-col items-center justify-center mt-10">
    ...
    <ul class="flex flex-col items-center justify-center mt-5">
      {#each songArtists as artist,i}
        <li class="w-full text-center m-1">
          <button on:click={handleClickArtist} data-index={i} class="btn btn-secondary btn-outline">{artist.name}</button>
        </li>
      {/each}
    </ul>
  </div>
